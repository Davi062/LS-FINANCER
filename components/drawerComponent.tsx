    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      {buttonTitle && (
        <DrawerTrigger asChild>
          <Button className="bg-[#236F5D] hover:bg-[#1a5446]" variant={buttonVariant} onClick={onOpen}>
            {buttonTitle}
        ...
        <div className={`mx-auto w-full ${size === 'sm' ? 'max-w-sm' : size === 'md' ? 'max-w-md' : size === 'lg' ? 'max-w-lg' : 'max-w-xl'}`}>
          ...
          <DrawerFooter>
            <div className="px-4 space-y-2">
              <Button className="bg-[#236F5D] hover:bg-[#1a5446] disabled:opacity-50 w-full" disabled={loading} onClick={onFooterActionClick}>
                {loading ? (
